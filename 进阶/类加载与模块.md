...
1. `l1`、`l2` 的 parent 是 bootstrap，`java.lang.Object` 仍是同一份。
2. 各自 `defineClass` 出两份 `Foo`。方法区里两个类，元数据两份。
3. `c1.isInstance(b)` 为 false：b 的 Class 不是 c1。
4. 用应用加载器的 `Foo` 去强转 l2 定义的实例，`ClassCastException`。异常消息里会写两个 Class 的加载器名字，看见就去查加载器，不要去查「是不是少写了 implements」。

这就是热加载、OSGi、Web 容器里「明明是同一个类」的根。
...
- `<clinit>` 里起线程、抢锁、调别的类的静态方法形成环。
- 线程池不传 TCCL，SPI 在池里找不到驱动。
