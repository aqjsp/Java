...
Map<Repo, String> m = new HashMap<>();
m.put(repo, "x");
System.out.println(m.get(repo));         // "x"，因为 hashCode/equals 按身份
```

1. `newProxyInstance` 校验 `Repo` 是接口，用给定加载器定义代理类，构造时把 handler 塞进 `Proxy.h`。
2. `repo.save("ab")` 字节码是 `invokeinterface`。代理类生成的 `save` 把调用收成 `handler.invoke(this, Method, new Object[]{"ab"})`。
3. `"ab"` 不用装箱（本来就是引用）。返回 `int` 2，代理类拆箱成 `ireturn`。
4. `toString` / `equals` / `hashCode` 若不在 handler 里特殊处理，默认也会进 `invoke`。用 `method.getName()` 漏了 `equals`，`HashMap` 的 key 会递归或恒 false。
5. 没有 `Repo` 的实现类对象。`repo.getClass().getSuperclass()` 是 `Proxy`，不是你的业务类。

把 `Repo` 换成 `class Repo { int save(String id) { return 0; } }` 再塞进 `newProxyInstance`，`IllegalArgumentException`。这就是「只能接口」。

---

## 六、反模式

- `Class.forName` 只为了得到 Class，却把驱动静态块跑了，静态块里连库。
- `getMethod` 找 private / 本类声明的非 public，找不到还以为没这个方法。
- 每个请求 `getDeclaredMethod` + `setAccessible` + `invoke`，不缓存。
- handler 里 `proxy.equals(x)` 再进 `equals`，栈溢出。
- 用 JDK Proxy 去代理具体类，失败了改引 CGLIB 还不理解 `final`。
- `invoke` 只 catch `Exception`，把 `InvocationTargetException` 当业务类型。
- `--add-opens` 打进 `java.base` 当生产默认，模块一收紧全炸。
- 热路径反射改 final 字段，对象已经发布。
