...
**单态调用点。** 同一个调用点 `a.speak()`，接收者静态类型是 `Animal`：

```java
void call(Animal a) { a.speak(); }   // 这一个调用点
```

| 阶段 | 这个调用点见过的类型 | JIT 做什么 |
| --- | --- | --- |
| 解释 / C1 | 还在计数 | 虚调用，查 vtable |
| 升温后只见过 Dog | {Dog} | **单态**：直接调 `Dog.speak`，可内联 |
| 来了第一个 Cat | {Dog, Cat} | 类型假设失败，**去优化** 回解释，再编双态 |
| 再来更多子类 | 多态 | 查 vtable / 接口 itable，内联可能撤掉 |

热点「突然变慢」常见原因是这张表从第二行走到第三行，不是 GC。CHA（类层次分析）在加载新的 `Cat` 时也会让「Animal 只有 Dog 一个实现」的假设失效，同样去优化。JFR 的 `Deoptimization` 事件能看见原因。

**逃逸分析**三种结果（实现优化，语言规则仍当对象在堆上）：栈上分配、标量替换、同步消除。`-XX:-DoEscapeAnalysis` 关掉后分配量上去。正确性不要依赖「一定没分配」。
