...
`javap -c Box` 能看见两个 `compareTo`：一个描述符 `(LBox;)I`，一个 `(Ljava/lang/Object;)I`。后者是合成桥，里面 `checkcast Box` 再 `invokevirtual compareTo:(LBox;)I`。反射 `getDeclaredMethods` 两个都在，`isSynthetic()` / `isBridge()` 为 true 的那个不是你写的。漏看桥，数目对不上，误以为自己写了重载。

协变返回同样生成桥：`Dog copy()` 覆盖 `Animal copy()`，桥方法返回 `Animal`、转调 `Dog.copy`。
