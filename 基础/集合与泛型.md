...
链表尾插时 `binCount` 从 0 起。桶里已有 1 个再加第 2 个，`binCount` 是 0；已有 8 个再加第 9 个，`binCount` 是 7。注释写了 `-1 for 1st`：阈值按「加上之后至少 8 个」来，**第 8 次 put 还不调 `treeifyBin`**。
...
final void treeifyBin(Node[] tab, int hash) {
    ...
    if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY)
        ...
        // 链表换成 TreeNode，再 hd.treeify(tab)
    }
}
```

`resize` 翻倍后，每个旧桶按 `hash & oldCap` 拆成 lo（原下标 `j`）和 hi（`j + oldCap`）。不必重算 hash。1.8 尾插不再成环，多线程仍会丢数据。
