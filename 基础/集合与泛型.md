...
### 3、`putVal` 走一遍：CAS 空桶、锁桶头、协助扩容

![CHM putVal：锁桶头，协助扩容](../image/java-chm-putval.svg)

OpenJDK 21 的 `spread`：`(h ^ (h >>> 16)) & 0x7fffffff`。比 HashMap 多一个 `& HASH_BITS`——最高位清零，因为负 hash 被 `MOVED(-1)`、`TREEBIN(-2)`、`RESERVED(-3)` 占了当标记。
...
for (Node<K,V>[] tab = table;;) {
    ...
    if (tab == null || (n = tab.length) == 0)
        tab = initTable();                          // CAS sizeCtl 抢初始化
    else if ((f = tabAt(tab, i = (n - 1) & h)) == null) {
        if (casTabAt(tab, i, null, new Node<>(h, key, value)))
            break;                                  // 空桶：CAS 挂上，无锁
    }
    else if ((fh = f.hash) == MOVED)
        tab = helpTransfer(tab, f);                 // 撞上 ForwardingNode，来帮搬
    else {
        synchronized (f) {                          // 只锁这条桶的头结点
            if (tabAt(tab, i) == f) {
                // fh >= 0 走链表尾插 / 覆盖；f instanceof TreeBin 走红黑树
            }
        }
    }
}
addCount(1L, binCount);                             // 分段计数，顺便看要不要扩容
```
...
| 撞到的桶 | 做什么 | 锁 |
| --- | --- | --- |
| 空 | `casTabAt(null → 新节点)` | 无锁，CAS |
| 链表头（`fh >= 0`） | `synchronized (f)` 后尾插或覆盖 | 只锁这一条桶 |
| `fh == MOVED`（ForwardingNode） | `helpTransfer` 去搬其它桶 | 协助扩容 |
| `fh == TREEBIN` | `synchronized (f)` 走红黑树 | 只锁这一条桶 |

**锁粒度是「一条桶的头结点」**，不是整表。别的桶照常读写。这是 8 相对 7 分段锁的进化：粒度从「16 段」细到「每个桶」。

**`sizeCtl`** 一个 int 编码四种状态：`-1` 正在初始化；负且非 -1 的值高位是 resize 戳、低位是「多少线程在协助扩容」；正数是下次扩容阈值；0 是默认。扩容时把旧表拆成一段段，每个线程 `transfer` 认领一段桶往新表搬，搬完的旧桶放一个 `ForwardingNode`（hash=MOVED）。别的线程 put 撞上 ForwardingNode，就 `helpTransfer` 一起搬——**扩容是多线程协作的**，不是一个线程闷头搬别人干等。

`addCount` 用类似 `LongAdder` 的 `CounterCell[]` 分段累加，`size()` 求和是估计。要长整数用 `mappingCount()`。

面试把 CHM 答成「把 HashMap 的方法都加 synchronized」是错的：锁的是桶头不是方法，扩容是协作的，计数是分段的，读多数时候无锁（`get` 全程不加锁，靠 `volatile` 的 `Node.val` 和 `tabAt` 的 acquire 语义）。
...
型变、通配符捕获、类型推断、递归泛型、反射拿真实类型参数，单独在 [泛型与类型系统](../进阶/泛型与类型系统.md) 里走读。这里只需记住：容器的类型参数擦成 `Object`，安全靠 `get` 处的 `checkcast`。
